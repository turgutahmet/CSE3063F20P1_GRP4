...
class GUI(Frame):
    ...
    def getFiles(self):
        ...
        addPolls = Button(self.frame2, height=2, width=30, text="Upload Polls", command=self.addStudentList,
                            background="SeaGreen",fg="white")
        addPolls.pack( expand=TRUE, padx=5, pady=5)
        addAnswerKey = Button(self.frame2, height=2, width=30, text="Upload Answer key", command=self.addStudentList,
                          background="SeaGreen",fg="white")
    ...
    def addStudentList(self):
        filename = filedialog.askopenfilenames( initialdir = "/",title="Select file",filetypes=(("exel files", "*.xlsx"), ("all files", "*.*")))
        self.studentList= self.parent.tk.splitlist(filename)

    def addPolls(self):
        filename = filedialog.askopenfilenames( initialdir = "/",title="Select file",filetypes=(("exel files", "*.xlsx"), ("all files", "*.*")))
        self.pollList= self.parent.tk.splitlist(filename)
    def addAnswerKey(self):
        filename = filedialog.askopenfilenames( initialdir = "/",title="Select file",filetypes=(("exel files", "*.xlsx"), ("all files", "*.*")))
        self.answerkeys= self.parent.tk.splitlist(filename)
